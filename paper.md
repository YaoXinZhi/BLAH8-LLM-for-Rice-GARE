...
Thanks to the platform provided by BLAH8, we were able to conduct this intriguing exploration of Rice-Alterome using LLMs. In future explorations, we plan to include the following points. Firstly, as reported in the paper's results evaluation section, directly assessing the results generated by GPT using evaluation metrics based on deep learning methods may likely result in numerous false negatives. This is because GPT often integrates more semantic information and adjusts boundaries of generated results, or the tense of words, which traditional deep learning evaluation methods do not consider. Therefore, there is a need to design evaluation methods specifically tailored to LLMs. On the other hand, this work only experimented within the context of rice, while the design of GARE is valuable for many other contexts, such as other animals or crops. Therefore, attempts to extend this project to other backgrounds would be meaningful. Finally, we plan to further evaluate the generated results and curate them manually before making them publicly available through web services, thereby promoting the use of literature resources in the field of precision breeding for rice.
